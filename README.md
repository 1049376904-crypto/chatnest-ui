...
这是一个个人 AI 聊天 Web App 的前端。手机优先，iOS 上「添加到主屏幕」当独立 App 用。
...
基于 [ugui3u/chatnest](https://github.com/ugui3u/chatnest) 二次开发，沿用它的 Non-Commercial 许可。原项目的记忆系统来自糖糖。

这一份改了挺多：界面从 2000 行长到 5900 行，砍掉了跟我个人生活绑死的那些功能，只留下对话、内置时钟、memories、diary 四件；另外加了记忆的近似去重和检索的新鲜度排序（下面都有）。
...
**内置时钟** — 每条发给模型的消息尾巴上挂一行「现在几点 + 对方多久没说话了」，模型不用调工具就知道时间。这一件在服务端，见下面。

**Memories** — Profile 页：名字、Saved memories（可逐条开关）、Preferences（自定义指令）、Memory summary（从最近几个对话框整理出来的长期印象，可看可改可关）。带近似去重，见下面。

**Diary** — 按月分组的日记列表 + 全文搜索；Calendar 视图按年铺开，每天记两个人的心情和一件事，点开就能改。
...
非商用。允许非商业复制、修改和再发布；禁止商业使用；署名不强制。
